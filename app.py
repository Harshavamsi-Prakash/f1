...
if st.button("Get Wind Speed and Weather Data"):
    ...
    if lat and lon:
        ...
        if data:
            ...
            # Scatter Plot of Weather Factors vs. Wind Speed
            st.subheader("Impact of Weather Factors on Wind Speed")
            selected_feature = st.selectbox("Select a feature to compare with wind speed", ["temperature", "humidity", "pressure", "precipitation", "cloud_cover"])
            fig_scatter = px.scatter(weather_df, x=selected_feature, y="wind_speed", title=f"{selected_feature.capitalize()} vs Wind Speed")
            st.plotly_chart(fig_scatter, use_container_width=True)

            # Time-Series Forecasting with Confidence Intervals
            st.subheader("Predicted Wind Speed with Confidence Intervals")
            fig_ci = go.Figure()
            for model_type, model in results:
                predicted_wind_speed = model.predict(future_data)
                df[f"Predicted Wind Speed (m/s) - {model_type}"] = predicted_wind_speed
                # Simulating confidence intervals (replace with actual model uncertainty if available)
                lower_bound = predicted_wind_speed - 1  # Example: ±1 m/s
                upper_bound = predicted_wind_speed + 1

                fig_ci.add_trace(go.Scatter(x=df["Time"], y=upper_bound, mode="lines", name=f"{model_type} Upper Bound", line=dict(dash="dot")))
                fig_ci.add_trace(go.Scatter(x=df["Time"], y=lower_bound, mode="lines", name=f"{model_type} Lower Bound", line=dict(dash="dot"), fill="tonexty"))
                fig_ci.add_trace(go.Scatter(x=df["Time"], y=predicted_wind_speed, mode="lines+markers", name=f"{model_type} Prediction"))

            fig_ci.update_layout(title="Predicted Wind Speed with Confidence Intervals", xaxis_title="Time", yaxis_title="Wind Speed (m/s)", template="plotly_white")
